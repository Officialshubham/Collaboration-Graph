...
function displayExternalCollaboration(){
    ...
    var simulation = d3.forceSimulation()
        .force("link", d3.forceLink().distance(300).strength(10.8).id(function (d) { return d.id; }))
        .force("charge", d3.forceManyBody())
        .force("center", d3.forceCenter(width / 2, height / 2));


    d3.json("https://raw.githubusercontent.com/Officialshubham/ProjectCollaborationGraph/master/project_external_new1.json", function (error, graph) {
...
function renderGraph(graph, flag) {
    ...
    // console.log(anuExternal);
    // console.log(externalColleges);
    ...
    originalGraph.links.forEach(function (link) {
        // nodeCountsExternal[link.source] = (nodeCountsExternal[link.source] || 0) + 1;
       var result1 = nodeCountsExternal[link.source] === undefined;
    //    var result2 = nodeCountsExternal[link.target] === undefined;

       if(result1){
        nodeCountsExternal[link.source] = 1;
       }
    //    if(result2){
    //     nodeCountsExternal[link.target] = 1;
    //    }
    ...
    // console.log(nodeCountsExternal);
    // // Calculate the count for each node based on the links
    // var nodeCountsExternal = {};
    ...
    if (flag == true) {
        ...
        var circles = node.append("circle")
            .attr("r", function (d) { return Math.sqrt(nodeCountsExternal[d.id]) * 3})
    ...
    node.append("title")
        // .text(function (d) { return d.name + " (" + d.school + ") "+ "Total Collaboration: "+nodeCountsExternal[d.id]; });
        .text(function (d) { 
            if(nodeCountsExternal[d.id]===undefined){
                return d.name + " (" + d.school + ") "+ "Total Collaboration: "+ 0; 
            }
            else{
                return d.name + " (" + d.school + ") "+ "Total Collaboration: "+nodeCountsExternal[d.id]; 
            }
           });
